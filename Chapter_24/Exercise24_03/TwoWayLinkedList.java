...
Description: A class for implementing a doubly linked list.   My modifications required
by the assignment are at lines 55, 74, 99, and 103
 */
...
class TwoWayLinkedList<E> extends java.util.AbstractSequentialList<E> {
	...
	public void addLast(E e) {
		...
		if (temp != null) {
			newNode.previous = temp; // Update the previous pointer of the new tail
		}
	}
	...
	public void add(int index, E e) {
		if (index == 0) {
			...
			if (temp != null) {
				temp.previous = current.next; // Update temp's previous pointer
			}
			// For a two-way linked list, link current's next's previous element to current
			current.next.previous = current; // Update the new node's previous pointer
		}
	}
