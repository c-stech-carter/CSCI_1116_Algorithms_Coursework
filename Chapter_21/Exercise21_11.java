...
public class Exercise21_11 extends Application {
    ...
    private void initializeData() {
        ...
        String baseUrl = "https://liveexample.pearsoncmg.com/data/babynamesranking";
        ...
        for (int yearIndex = 0; yearIndex < 10; yearIndex++) {
            int year = 2001 + yearIndex; // Years 2001 to 2010
            String url = baseUrl + year + ".txt";

            try (Scanner input = new Scanner(new URL(url).openStream())) {
                int rank = 1;
                while (input.hasNextLine() && rank <= 10) { // Read only the first 10 ranks
                    String line = input.nextLine();
                    String[] tokens = line.split("\\s+"); // Split by spaces

                    if (tokens.length >= 4) {
                        String boyName = tokens[1];   // Boy's name
                        String girlName = tokens[3];  // Girl's name

                        // Populate the maps
                        mapForBoy[yearIndex].put(boyName, rank);
                        mapForGirl[yearIndex].put(girlName, rank);

                        rank++;
                    }
                }
            } catch (IOException e) {
                System.out.println("Error reading data for year " + year + ": " + e.getMessage());
            }
        }
    }
